...
describe("tryCatch", () => {
  it("Should return promise value if the promise resolves", async () => {
    const promiseFn = vi
      .fn()
      .mockImplementationOnce(async () => Promise.resolve({ hey: "Bedesqui" }))

    const [data, error] = await tryCatch(promiseFn())
  ...
  it("Should return error if the promise rejects with an Error value", async () => {
    const promiseFn = vi
      .fn()
      .mockImplementationOnce(async () => Promise.reject(new Error("I'm a failure")))
  ...
  it("Should throw if the promise rejects with an non-Error value", async () => {
    ...
    await tryCatch(promiseFn()).catch((throwable) => {
